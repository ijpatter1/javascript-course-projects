...
function watch() {
    ...
    btnDisabled(btn);
} // disables the stop button since the game has not started
...
function startGame() {
    ...
    btnDisabled(btn); // disable start button while game running
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); // enable stop button while game running
...
// these functions style the game buttons while they are disabled
...
function stopGame() {
    ...
    var btn = document.getElementById('btnStart');
    startEnabled(btn); //enable the start btn while the game is stopped
    var btn = document.getElementById('btnStop');
    btnDisabled(btn); //disable stop btn while the game is stopped
    var showPlayer = document.getElementById('showPlayer')
...
function saveSettings() {
    ...
    if (p1Selected[p1Index].text == p2Selected[p2Index].text) {
        alert('Error - Player 1 and Player 2 cannot be both assigned as: '+p1Selected[p1Index].text);
    } else {
...
function recordMoves(square) {
	...
	verdict = check(info,proposedMove); // call function to check if proposed square is already occupied
	return verdict;
}
